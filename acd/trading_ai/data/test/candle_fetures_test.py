...
from data_faker import create_gap_test_scenarios,create_candle_test_scenarios,different_candles_logs,get_fake_data
def display_correlation_map(df,candidate_features,title):
            correlation_matrix = df[candidate_features].corr(numeric_only=True,method='pearson')
            ...
            plt.title(f"Feature Correlation Heatmap for {title}")
            plt.tight_layout()
            plt.show()
...
#--------------------------------------------------------------------------------------
def candle_feature_tests() -> None:
    test_df = create_candle_test_scenarios()
    result_df = add_candle_features(test_df)
    df = result_df.set_index("Scenario")
    ...
    display_correlation_map(df,candidate_features,'candle_features')


def gap_feature_tests():
        test_df = create_gap_test_scenarios()
        result_df = add_gap_features(test_df)

        feature_candidate = [
            "GapPct",
            "GapDirection"
        ]
        display_correlation_map(result_df,feature_candidate,'gap_feature')

########### log features

def log_feature_test():
      result_df = different_candles_logs()
      result_df=add_log_returns(result_df)
      feature_candidates = [
        "Scenario",
        "CandleShape",
        "datetime",
      ...
      display_correlation_map(result_df,feature_candidates,'log')


def volume_features_test():
     df=get_fake_data()
     ...
     print()
     df.head()
     display_correlation_map(df,feature_candidate,'volume')


def volatility_features_test():
     df=get_fake_data()
    ...
    "TrueRangePct",
     ...
     print(feature_candidate)
     df.head()
     display_correlation_map(df,feature_candidate,'volatility')      
def trending_features_test():
     df=get_fake_data()
    ...
    'Trend5']
     ...
     print(f)
     df.head()
     display_correlation_map(df,feature_candidate,'trending')

def rolling_statistics_test():
     df=get_fake_data()
    ...
    "RollingKurtosis"
     ...
     print(f)
     df.head()
     display_correlation_map(df,feature_candidate,'rolling')
def zscore_features_test():
     df=get_fake_data()
     df=add_log_returns(df)
     df=add_zscore_features(df)
     f=df.columns.tolist()
     candidate_features=['volume', 'LogReturn1', 'CloseZ', 'VolumeZ', 'ReturnZ']
     print(f)
     display_correlation_map(df,candidate_features,'z-score')            
...
def all_features_test():
     ...
     feature_candidate=[##### from log feature
         ...
         ###### body
          "BodyPct","RangePct","UpperShadowPct","LowerShadowPct","BodyToRange",
                     "Bullish", 
                     "StrangeFlatCandle", "InactiveFlatCandle",
                     # "ZeroVolume",
          ######### gap features
           "GapPct","GapDirection",
           ##### trending features
    ...
    #  df.to_csv(Path(__file__).resolve().parent / "feature_added_fake_data.csv", index=False)
     
     display_correlation_map(df,feature_candidate,'all-features')
     selected_features = [
                            # Returns / momentum target classes
                            # "LogReturn1",
                            # "LogReturn3",
                            # "LogReturn10",
                            ...
                            # Standardized price/return
                            "CloseZ",
                            "ReturnZ",]

     less_complex_matrix = ['BodyPct',
            'EMA_Ratio',
            'GapPct',
            'LowerShadowPct',
            'RollingKurtosis',
            ...
            'smh_VolumeChange',
            'smh_GapDirection',
            'spy_GapDirection']  # Features you want to check

     ## we need to remove warm up candles 
     # This is preferable to blindly writing: df = df.iloc[20:]
    # Important: with multiple stocks, each ticker has its own 
    # first 20 warm-up rows. Therefore, do not remove only the first 20 rows of the combined dataset.
    #  Calculate features separately per ticker and then remove rows containing NaN from the
    #  selected features.
     # 
     display_correlation_map(df,selected_features,'all-features')


###### check feature correlation 
# candle_feature_tests()
# gap_feature_tests()
##### log---------------
# log_feature_test()
#-------------- 
# volume_features_test()
# volatility_features_test()
# trending_features_test()
